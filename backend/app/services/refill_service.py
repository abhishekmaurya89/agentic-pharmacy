...
async def predict_refill(
    ...
    if not intervals:
        return {
            "predictable": False,
            "reason": "Unable to calculate purchase interval"
        }
...
async def generate_refill_alert(
    ...
    prediction = await predict_refill(
        patient_id,
        medicine_id
    )
    ...
    alert = {
        "patient_id": patient_id,
        "medicine_id": medicine_id,
        "predicted_refill_date": prediction[
    ...
    result = await db.refill_alerts.insert_one(
        alert
    )
