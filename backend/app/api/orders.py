...
async def get_order_status(
    ...
    if order.get("patient_id") != current_user["id"]:
        raise HTTPException(
            status_code=403,
...
async def create_order(
    ...
    if current_user["role"] != "patient":
        return {"error": "Only patients can place orders"}

    if len(order.items) != 1:
        return {"error": "MVP currently supports one medicine per order"}
    ...
    return await execute_order(
        patient_id=patient_id, medicine_id=item.medicine_id, quantity=item.quantity
    )
