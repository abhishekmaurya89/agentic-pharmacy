...
def build_pharmacy_graph():
    ...
    graph.add_node("medicine_information", medicine_information)

    graph.add_node("unknown_request", unknown_request)
    ...
    graph.add_conditional_edges(
        "assess_risk",
        route_risk,
        {"patient": "prepare_order", "pharmacist": "pharmacist_review"},
    )
    # Prepare
    ...
    # Approval
    graph.add_conditional_edges(
        "human_approval", route_approval, {"execute": "execute_order", "cancel": END}
    )
